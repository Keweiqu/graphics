...
void triangulate(Polygon* polygon){
  unsigned int size = count_polygon_sides(polygon);
  if(size <= 3){
    return;
  }
  while(size > 3){
    unsigned int i = 0;
    int left_index = i - 1 < 0 ? i - 1 + size: i - 1;
    int right_index = i + 1 > size - 1? i + 1 - size: i + 1;
...
int is_ear(unsigned int index,unsigned int size, Polygon* polygon) {
  Point* ear = polygon->cache[index];
  int left_index = i - 1 < 0 ? i - 1 + size: i - 1;
  int right_index = i + 1 > size - 1? i + 1 - size: i + 1;
  Point* left = polygon->cache[left_index];
  Point* right = polygon->cache[right_index];
  GLfloat slope, intercept;
  calc_line(left, right, &slope, &intercept);
  int above = ear->x * slope + intercept < ear->y ? TRUE: FALSE;
  
}

void calc_line(Point* p, Point q, GLfloat* slope, GLfloat* intercept) {
  *slope = (p->y - q->y) / (p->x - q->y);
  *intercept = p->y - *slope * p->x;
}

int in_triangle(Polygon* polygon, unsigned int index, int left, int right){
